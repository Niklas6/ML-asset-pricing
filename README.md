...
The project uses a walk-forward prediction design rather than a random train/test split. At each annual step, the models are trained on the previous 20 years of monthly stock observations and then used to predict the next 12 month. The models are refit once per year, so the evaluation mimics a researcher updating a model through time using only past data.
...
- The stock universe is manually selected and therefore subject to survivorship bias.
-  Yahoo Finance data is accessible, but not institutional-grade.
- The risk-free-rate proxy is an approximation and may not perfectly match assumptions used in a real portfolio.
- The backtest is idealized at the moment. 
- The model comparison is based on a limited set of fixed hyperparameter choices after validation. A broader search and combination of models could improve the performance and is a planned improvement. 
- The results should be interpreted as a research prototype. 



## Summary

Overall, the results suggest that simple machine learning models can extract some predictive information from price-based equity features. In an idealized backtest, this signal translates into positive long-short returns before transaction costs. However, the signal is not stable across market regimes, and the strategy would need substantial improvements before it could be interpreted as practically tradable.
...
The main takeaway is that the project provides a reproducible framework for empirical asset-pricing research: building a monthly stock panel, running walk-forward validation, comparing model classes, and testing whether predictive signals survive a simple portfolio construction step.
